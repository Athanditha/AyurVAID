...
def train():
    ...
    # Initialize the CatBoost model which learns sequentially from its mistakes
    model = CatBoostClassifier(
        iterations=100,         # Number of gradient boosted trees
        learning_rate=0.1,      # Step size taken to correct errors per tree
        depth=5,                # Max depth per tree
        loss_function='MultiClass', # Multi-class classification returning probabilities
        random_seed=42,
        verbose=0
    )
